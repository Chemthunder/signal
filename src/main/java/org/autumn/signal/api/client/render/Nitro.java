package org.autumn.signal.api.client.render;
...
public class Nitro {
    ...
    public static void texSphere(MatrixStack matrices, VertexConsumer vertices, double x, double y, double z, float radius, int quality, float rotation) {
        ...
        for(int i = 0; i < quality; ++i) {
            ...
            for(int j = 0; j < quality; ++j) {
                ...
                quad(matrices.peek(), vertices, x1, y1, z1, x2, y1, z2, x3, y3, z3, x4, y3, z4, u1, v1, u2, v1, u2, v3, u1, v3);
            }
        }
    }

    public static void skyBeam(MatrixStack matrices, VertexConsumer vertices, double x, double y, double z, float radius, int height, float time) {
        MatrixStack.Entry entry = matrices.peek();
        float xP = (float) (x + 0.5F);
        float zP = (float) (z + 0.5F);
        float yStart = (float) y;
        float yEnd = yStart + (float)height;
        float angle = time * 0.2F % 1.0F;
        float[][] corners = new float[][]{{xP - radius, zP - radius}, {xP + radius, zP - radius}, {xP + radius, zP + radius}, {xP - radius, zP + radius}};
        float u0 = 0.0F;
        float u1 = 1.0F;
        float v0 = angle;
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            beamQuad(entry, vertices, corner1[0], yStart, corner1[1], corner2[0], yStart, corner2[1], corner2[0], yEnd, corner2[1], corner1[0], yEnd, corner1[1], u0, v0, u1, v1);
        }
    }

    public static void cone(MatrixStack matrices, VertexConsumer vertices, double x, double y, double z, float radius, int height, float time) {
        MatrixStack.Entry entry = matrices.peek();
        float xP = (float) x + 0.5F;
        float zP = (float) z + 0.5F;
        float yBase = (float) y;
        ...
        for (int i = 0; i < segments; ++i) {
            ...
            float x1 = xP + (float) Math.cos(angle1) * radius;
            float z1 = zP + (float) Math.sin(angle1) * radius;
            float x2 = xP + (float) Math.cos(angle2) * radius;
            float z2 = zP + (float) Math.sin(angle2) * radius;
            float v1 = angleOffset + (float) height / 8.0F;
            beamQuad(entry, vertices, x1, yBase, z1, x2, yBase, z2, xP, yTip, zP, xP, yTip, zP, u0, angleOffset, u1, v1);
        }
    }

    public static void solColCube(MatrixStack matrices, VertexConsumer vertices, int color, double x, double y, double z, float inflation) {
        MatrixStack.Entry entry = matrices.peek();
        float x0 = (float)(x + (double)0.5F - (double)inflation);
        float x1 = (float)(x + (double)0.5F + (double)inflation);
        float y0 = (float)(y + (double)0.5F - (double)inflation);
        float y1 = (float)(y + (double)0.5F + (double)inflation);
        float z0 = (float)(z + (double)0.5F - (double)inflation);
        float z1 = (float)(z + (double)0.5F + (double)inflation);
